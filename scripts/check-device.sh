...
echo "3. IMPORTANT - open Noticon's detail page and tap Apply once."
echo "   Some managers (Vector 2.2 confirmed) do not write scope.list into"
echo "   their own scope database for staticScope=true modules, so the module"
echo "   shows as enabled yet is never injected. Tapping Apply commits it."
echo "   Redo this after uninstalling the app: a fresh install gets a new"
echo "   /data/app path and invalidates the previous record."
echo "   (adb install -r over an existing install keeps the path - no need.)"
echo "4. Optional: open Noticon in the launcher to pick an icon mode."
echo "   Default is monochrome when the app was never opened."
echo "==============================================="
